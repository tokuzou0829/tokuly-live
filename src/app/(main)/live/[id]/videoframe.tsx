      ...
      if (status !== "online") {
        ...
        if (newLivedata.status === "end" && !archivecheckstatus) {
          const checkvideo = await fetch(
            `https://live-data.tokuly.com/videos/hls/${live.stream_name}/index.m3u8`,
            {
              method: "GET",
              headers: {},
            }
          ...
          if (checkvideo.ok) {
            ...
            if (playlistMatches) {
              ...
              const response = await fetch(
                `https://live-data.tokuly.com/videos/hls/${live.stream_name}/${playlistUrl}`
              );
              if (!response.ok) {
        ...
        >
          {isArchive ? (
            <Link href={`/video/${live.stream_name}`}>
              <div className="bg-black bg-opacity-50 p-4 rounded-lg cursor-pointer transition-all duration-300 flex flex-col items-center">
