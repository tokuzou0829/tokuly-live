...
export const useCanvas = (
  ...
  const drawOverlay = useCallback(
    (overlayCtx: CanvasRenderingContext2D, source: VideoSource) => {
      if (source.id === selectedSourceId) {
        ...
        overlayCtx.fillRect(source.x + source.width - 15, source.y + source.height - 15, 15, 15);
      }
    },
    [selectedSourceId]
  );
  ...
  const handleCanvasMouseDown = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      const sources = [...videoSourcesRef.current].sort(
        (a, b) =>
          videoSourcesRef.current.length -
          1 -
          videoSourcesRef.current.findIndex((s) => s.id === b.id) -
          (videoSourcesRef.current.length -
            1 -
            videoSourcesRef.current.findIndex((s) => s.id === a.id))
      );
  ...
  const handleCanvasMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>, resizeInfo: ResizeInfo | null) => {
        ...
        switch (resizeInfo.edge) {
          ...
          case "right":
            newCrop = {
              ...newCrop,
              width: Math.min(
                maxWidth - source.crop!.x,
                Math.max(100, resizeInfo.startWidth + deltaX)
              ),
            };
            newCrop.height = newCrop.width / source.cropAspectRatio!;
            break;
          ...
          case "bottom":
            newCrop = {
              ...newCrop,
              height: Math.min(
                maxHeight - source.crop!.y,
                Math.max(100, resizeInfo.startHeight + deltaY)
              ),
            };
            newCrop.width = newCrop.height * source.cropAspectRatio!;
            break;
  ...
  const updateCursor = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {
      ...
      if (source.isCropping) {
        if (
          (isNearLeftEdge && isWithinVerticalBounds) ||
          (isNearRightEdge && isWithinVerticalBounds)
        ) {
          e.currentTarget.style.cursor = "ew-resize";
        } else if (
          (isNearTopEdge && isWithinHorizontalBounds) ||
          (isNearBottomEdge && isWithinHorizontalBounds)
        ) {
